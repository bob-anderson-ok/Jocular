...
public class ChartZoomManager {
	...
	private final BooleanProperty zoomAnimated = new SimpleBooleanProperty( false );
	...
	public ChartZoomManager( Pane chartPane, Rectangle selectRect, XYChart<?,?> chart ) {
		...
		handlerManager = new EventHandlerManager( chartPane );
                ...
                handlerManager.addEventHandler(false, MouseEvent.MOUSE_PRESSED, 
                        mouseEvent -> {if ( passesFilter(mouseEvent) ) onMousePressed(mouseEvent);}
                );

		handlerManager.addEventHandler(false, MouseEvent.DRAG_DETECTED, 
                        mouseEvent -> {if ( passesFilter(mouseEvent) ) onDragStart();}
                );
                
                handlerManager.addEventHandler(false, MouseEvent.MOUSE_DRAGGED, 
                        mouseEvent -> onMouseDragged( mouseEvent)
                );
                
                handlerManager.addEventHandler(false, MouseEvent.MOUSE_RELEASED, 
                        mouseEvent -> onMouseReleased()
                );
