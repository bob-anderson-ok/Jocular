...
/**
 * ChartPanManager manages drag gestures on an {@link XYChart} by translating
 * them to panning actions on the chart's axes.
 *
...
public class ChartPanManager {
    ...
    /**
     * The default mouse filter for the {@link ChartPanManager} filters events
     * unless only primary mouse button (usually left) is depressed.
     */
    ...
    /**
     * Sets the mouse filter for starting the pan action. If the filter consumes
     * the event with {@link Event#consume()}, then the event is ignored. If the
     * filter is null, all events are passed through. The default filter is
     * {@link #DEFAULT_FILTER}.
     */
